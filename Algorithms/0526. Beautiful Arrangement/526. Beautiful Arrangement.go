...
func countArrangement(N int) int {
	...
	generatePermutation_526(nums, 0, p, &res, &used)
	return len(res)
}

func generatePermutation_526(nums []int, index int, p []int, res *[][]int, used *[]bool) {
	if index == len(nums) {
		temp := make([]int, len(p))
		copy(temp, p)
		*res = append(*res, temp)
	...
	for i := 0; i < len(nums); i++ {
		if !(*used)[i] {
			...
			(*used)[i] = true
			p = append(p, nums[i])
			generatePermutation_526(nums, index+1, p, res, used)
			p = p[:len(p)-1]
			(*used)[i] = false
		}
	}
