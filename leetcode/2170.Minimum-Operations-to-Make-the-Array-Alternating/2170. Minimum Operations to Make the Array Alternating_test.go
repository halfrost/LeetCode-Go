...
func Test_Problem1(t *testing.T) {
	...
	qs := []question2170{
		...
		// even has only one distinct value equal to odd[0], odd has multiple
		{
			para2170{[]int{2, 2, 2, 3, 2, 2}},
			ans2170{2},
		},

		// both multiple, even[0]==odd[0], asymmetric counts (min picks the smaller branch, a > b true)
		{
			para2170{[]int{1, 1, 1, 1, 1, 1, 1, 2, 1, 2, 9, 3}},
			ans2170{5},
		},

		// both even and odd have a single distinct value, equal
		{
			para2170{[]int{5, 5, 5, 5}},
			ans2170{2},
		},

		// even[0].value == odd[0].value, both have multiple distinct values (hits min)
		{
			para2170{[]int{1, 1, 2, 1, 1, 2}},
			ans2170{2},
		},
